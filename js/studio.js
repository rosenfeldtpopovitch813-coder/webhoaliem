...
(function(){
 ...
 function mount(){
  ...
  const shortcuts=node('div','st-shortcuts');for(const [label,desc,key,fn] of [['Luyện một đề','Kiểm tra điều đã học','exam',()=>loadExamList()],['Mở thư viện','Tìm tài liệu bạn cần','library',()=>switchSection('sec-library')],['Kế hoạch nguyện vọng','Chọn hướng đi tiếp theo','compass',()=>HLLearning.open('plan')]]){const b=action(label,key,fn,'st-shortcut');b.append(node('small','',desc));shortcuts.append(b);}
