...
const {extractText}=require('../lib/ai-response');
module.exports = async function handler(req, res) {
  ...
  // Per-account quota is stored server-side, so concurrent instances share a limit.
  try {
    const app=getAdmin(),user=await verifyUser(req),now=Date.now();
    if(JSON.stringify(req.body||{}).length>3500000)return res.status(413).json({error:'Ảnh hoặc nội dung quá lớn.'});
    if(typeof req.body?.prompt!=='string'||req.body.prompt.length>12000)return res.status(400).json({error:'Nội dung không hợp lệ.'});
    const quota=await app.database().ref('apiRateLimits/'+user.uid+'/ai').transaction(value=>{
      const state=value&&value.resetAt>now?value:{count:0,resetAt:now+60000};
      if(state.count>=6)return;return {...state,count:state.count+1};
    });
    if(!quota.committed)return res.status(429).json({error:'Bạn gửi hơi nhanh. Hãy thử lại sau một phút.'});
  } catch(error) {console.error('AI admission',error);return res.status(error.status||503).json({error:'AI chưa sẵn sàng. Các công cụ học tập vẫn dùng được.'});}
  ...
  try {
    ...
    return res.status(500).json({ error: 'AI tạm thời không khả dụng. Hãy thử lại sau.' });
  }
};
