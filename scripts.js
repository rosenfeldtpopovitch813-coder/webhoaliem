...
function esc(v){ return HL.esc(v); }
...
function initOnlinePresence(user){
  try{
    ...
    const ref = window.db.ref('presence/'+user.uid+'/'+(window.__presenceTab||(window.__presenceTab=crypto.randomUUID())));
...
function initFirebase() {
    ...
    window.__firebaseReadyPromise = (async () => {
        ...
        window.dispatchEvent(new CustomEvent('hl:auth-change',{detail:{uid:user?.uid||null}}));
        const welcome = document.getElementById('sec-welcome');
        const login = document.getElementById('sec-login');
        ...
        if (user) {
            ...
            clearOnlinePresence();
            bindStudyRealtime(null);
            document.querySelectorAll('.app-section').forEach(el=>{el.classList.add('hidden');el.classList.remove('active');});
...
function loginWithGoogle() {
    ...
    if(!auth||!provider){if(errObj){errObj.textContent='Kết nối chưa sẵn sàng. Hãy tải lại trang.';errObj.classList.remove('hidden');}return;}
    auth.signInWithPopup(provider).catch(error => {
        if(errObj) {
            console.warn("Google login",error); errObj.innerText = "Không thể đăng nhập. Cho phép cửa sổ đăng nhập rồi thử lại.";
...
function __statsDateKey(d){ return HL.dayKey(d); }
...
async function openStats() {
    ...
    try {
        ...
        loading.textContent = 'Không thể tải thống kê. Hãy thử lại khi có kết nối.';
    }
}
...
document.addEventListener("DOMContentLoaded", () => { 
    initFirebase().catch(()=>{const e=document.getElementById('firebase-error');e?.classList.remove('hidden');document.getElementById('firebase-error-text').textContent='Không thể kết nối. Kiểm tra mạng rồi tải lại trang.';}); 
...
function loadDocuments(){if(typeof Papa==='undefined'){window.__docsLoaded=false;document.getElementById('document-grid').textContent='Chưa tải được bộ đọc tài liệu. Kiểm tra mạng rồi tải lại trang.';return;}const uid=auth?.currentUser?.uid;Papa.parse(SHEET_CSV_URL,{download:true,header:true,complete:r=>{if(auth?.currentUser?.uid!==uid)return;docsData=r.data;renderDocs(docsData);},error:()=>{window.__docsLoaded=false;document.getElementById('document-grid').textContent='Không thể tải tài liệu. Hãy thử lại khi có kết nối.';}});}
...
function buildTeacherSheet(){
    ...
    if(!HL.validateConfig(examConfig)) return alert('Số câu hoặc thời gian không hợp lệ.');
    examConfig.pdfUrl=HL.safeURL(examConfig.pdfUrl);
    correctAnswers={};
...
function updateChemPreview() { clearTimeout(chemTimeout); const rawText = document.getElementById('chem-editor').value.normalize('NFC'); document.getElementById('chem-pdf-content').replaceChildren(HL.sanitizeChem(rawText)); chemTimeout = setTimeout(() => { if (window.MathJax?.typesetPromise) MathJax.typesetPromise([document.getElementById('chem-pdf-content')]); }, 300); }
...
(function(){
  ...
  const dateKey = (d=new Date()) => HL.dayKey(d);
  ...
  async function syncStudyStats(){ return window.HLStudy?.sync(); }
  ...
  function bindStudyRealtime(user){
    ...
    totalStudySeconds=0;localStorage.setItem(STORE.todaySeconds,'0');localStorage.setItem(STORE.sessions,'0');
    ...
    studyCloudRef.on('value',snap=>{
      try{
        ...
        totalStudySeconds=cloudTotal;
        localStorage.setItem(STORE.todaySeconds,String(cloudToday));
        localStorage.setItem(STORE.sessions,String(cloudSessions));
  ...
  window.buildTeacherSheet=function(){
    ...
    Object.entries(saved||{}).forEach(([q,v])=>{
      if(q.startsWith('short_')){const el=document.getElementById('input-'+q);if(el){el.value=v;correctAnswers[q]=v;}}
      else document.querySelector(`input[name="${CSS.escape(q)}"][value="${CSS.escape(v)}"]`)?.click();
    });
  };
...
(function(){
  ...
  async function makeUniqueRoomCode(){
    for(let i=0;i<8;i++){
      ...
      const snap=await HL.roomSnapshot(code);
  ...
  window.__createRoomV2 = async function(){
    ...
    if(!HL.validateConfig(examConfig))return alert('Hãy tạo phiếu trả lời hợp lệ trước.');
  ...
  window.prepareJoin=async function(id){
    currentRoomId=String(id||'').trim();
    if(!/^[A-Za-z0-9_-]{1,128}$/.test(currentRoomId)) return alert('Mã phòng không hợp lệ.');
    try{
      const s=await HL.roomSnapshot(currentRoomId);
...
// ==========================================
function toggleTheme(){const dark=document.body.classList.toggle('hl-dark');localStorage.setItem('htvvm_theme',dark?'dark':'light');const b=document.getElementById('theme-toggle-btn');if(b)b.textContent=dark?'☀️':'🌙';}
document.addEventListener('DOMContentLoaded',()=>{document.body.classList.toggle('hl-dark',localStorage.getItem('htvvm_theme')==='dark');});
...
async function initCalendarEventsForUser(user){
  const uid=user?.uid;
  if(uid && calendarEventsBoundForUid===uid) return;
...
window.uploadCustomAvatar = function(e) {
    const file = e.target.files[0], avatarOwner=auth?.currentUser?.uid;
    if (!file || !avatarOwner || !/^image\//.test(file.type) || file.size>4*1024*1024) return;
    ...
    reader.onload = function(event) {
        ...
        img.onload = function() {
            ...
            db.ref('users/' + avatarOwner).update({ customAvatar: base64Avatar })
              .then(() => {
                  if(auth?.currentUser?.uid!==avatarOwner)return;
...
(() => {
  ...
  function getBreakState(){ return HL.storage.get(BREAK_STORE+'.'+(auth?.currentUser?.uid||'guest'),{}); }
  function setBreakState(x){ HL.storage.set(BREAK_STORE+'.'+(auth?.currentUser?.uid||'guest'),x); }
  ...
  function getImage(){ return localStorage.getItem(BREAK_STORE+'.image') || DEFAULT_BREAK_IMG; }
  ...
  function openBreakPrompt(){
    ...
    window.HLStudy?.pauseForConfirmation();
  ...
  window.continueAfterStudyBreak=function(){ window.HLStudy?.continue(); const st=getBreakState(); st.fiveHourShown=true; setBreakState(st); closeBreak(); };
  window.stopAfterStudyBreak=function(){ window.HLStudy?.stop();
...
(function(){
  ...
  function awardStudySeconds(){ window.HLStudy?.sync(); }
...
(() => {
  ...
  async function arenaRoomJoinPublic(code){
    ...
    if(r.players?.[u.uid])await ref.child(`players/${u.uid}/online`).set(true);else await ref.child(`players/${u.uid}`).set(upd); await ref.child(`logs/${Date.now()}`).set(`👤 ${u.name} đã vào phòng.`);
    return r;
  }

  window.arenaJoinRoom=async function(code){
    if(!/^[A-Z0-9]{6}$/.test(String(code)))return alert('Mã phòng không hợp lệ.');
    if(!(await ensureArenaAuth()))return;
  ...
  window.arenaAnswer=async function(option){
    ...
    if(now()>r.deadline||!r.players?.[u.uid]||!q.a?.[option])return;
    const correct=String(option)===String(q.c);
    await DB().ref(path).transaction(value=>value||{option,correct,answeredAt:now()});
    A("arena-turn-label").textContent="ĐÃ TRẢ LỜI";
  };

  async function maybeHostResolve(){
    const index=arenaRoomCache?.currentIndex,u=me();if(!arenaRoomRef||!u)return;
    try{await arenaRoomRef.transaction(current=>HL.resolveArena(current,u.uid,now(),index));}catch(error){console.warn('Arena resolve',error);}
  }
  ...
  window.arenaLeaveRoom=function(){if(arenaRoomRef&&me())arenaRoomRef.child(`players/${me().uid}/online`).set(false).catch(()=>{});detachArena();arenaRoomCode="";arenaRoomCache=null;history.replaceState(null,"",location.pathname);openChemArena()};
  ...
  setInterval(()=>{if(arenaRoomCache?.phase==="playing"&&A("arena-live-timer"))A("arena-live-timer").textContent=formatSecs((arenaRoomCache.deadline-now())/1000)},500);
