...
const api=new Set(['time','rooms','leaderboards','study-time','exam','gemini']);
...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  res.setHeader('Content-Type',mime[extension]);res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Cache-Control','no-cache');if(req.method==='HEAD')res.end();else fs.createReadStream(file).pipe(res);
